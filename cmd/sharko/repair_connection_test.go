...
func TestRepairConnection_DoesNotRepairWhatItMayNotRepair(t *testing.T) {
	startCLITestServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		writeCLIJSON(t, w, map[string]interface{}{
			...
			// No limit_reason: an ownership conflict carries none. The typed
			// status, scope and ownership_mode are the whole answer, and each
			// surface phrases it once in its own register. This fixture used to
			// hand-type a sentence here, which described a response the server
			// can no longer send.
