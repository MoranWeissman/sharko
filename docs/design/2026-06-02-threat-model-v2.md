...
>
> This file keeps the filename and URL of the old v2.0.0 threat model
> so that every existing link to it (the README, the operator security
> page) keeps working. The content below is a full rewrite for v4 — it
> is not an update of the old STRIDE/OWASP-style document, it is a
> different, plainer document that replaces it.
...
`DELETE /api/v1/addon-secrets/{addon}` are gone — the whole in-memory
definition map they wrote to no longer exists in the production write
path. `POST /clusters/{name}/secrets/refresh` no longer reads a
