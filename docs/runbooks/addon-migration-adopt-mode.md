...
## Version Differences Between OLD and NEW ArgoCD

The following addons have **different versions** in the NEW catalog compared to the OLD ArgoCD. If you need to keep the OLD version during migration, you must explicitly set it in the cluster labels (e.g., `istiod-version: "1.22.0"`) in `configuration/cluster-addons.yaml`.

| Addon | OLD Version | NEW Version |
|-------|------------|------------|
| datadog | 3.70.7 | 3.160.1 |
| external-secrets | 0.9.10 | 0.16.2 |
| istio-base | 1.22.0 | 1.28.0 |
| istiod | 1.22.0 | 1.28.0 |
| istio-cni | 1.22.0 | 1.28.0 |
| istio-ingress | 1.22.0 | 1.28.0 |

All other addons (keda, redisinsight, redisinsight-v2, kafka-ui, secrets-store-csi-driver, sscdp-aws, anodot, kyverno, argo-rollouts, karpenter, cert-manager, argo-events, argo-workflows, external-dns) have **matching versions** and require no special handling.

> **Important:** During migration, values must match exactly (see Step 2). If the NEW catalog has a newer version, either:
> 1. **Pin the OLD version** via cluster label override (e.g., `datadog-version: "3.70.7"`) to migrate first, then upgrade separately
> 2. **Accept the upgrade** if the version bump is intentional and tested
...
**Fix:**
Verify ApplicationSet template (`bootstrap/templates/addons-appset.yaml`) has the `inMigration` logic that injects `migrationIgnoreDifferences` when `$appset.inMigration` is true. Also verify `migrationIgnoreDifferences` is defined in `configuration/addons-catalog.yaml`.
