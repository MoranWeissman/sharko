...
The full developer guide — covering build, project layout, key packages, the curated catalog, AI annotation pipeline, security primitives, supply-chain signing, and "how to add an X" recipes — lives at the repository root so it stays close to `go.mod`, `Makefile`, and the source tree it documents.
...
- **Curated Catalog** — embedded YAML, loader, search, scorecard refresh, REST surface, ArtifactHub proxy/cache, security primitives.
...
New UI surfaces target **WCAG 2.1 AA** — keyboard navigation, focus rings on every interactive element, semantic landmarks (`role="navigation"` / `role="main"`), and contrast ratios that pass `axe-core` with zero violations on every page in the v2 baseline.
