...
- **`sts:TagSession`, not just `sts:AssumeRole`.** If Sharko runs with EKS Pod Identity and you use per-cluster Role ARNs, the assume-role hop needs `sts:TagSession` granted on BOTH sides, in addition to `sts:AssumeRole`: the pod's own role needs `sts:AssumeRole` + `sts:TagSession` in its permission policy for the target role, AND the target role's trust policy needs to allow both actions from that principal — Pod Identity sessions are tagged, and AWS rejects the assume without it. This bit the live run with `AccessDenied: ... not authorized to perform: sts:TagSession`; it's a real-user landmine, not something specific to this test harness, so check both sides (the assuming role's permission policy and the target role's trust policy) whenever you wire up cross-role EKS access under Pod Identity.
