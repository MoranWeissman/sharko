...
3. **If the TUF fetch returns a malformed body, you cannot point Sharko
   ...
   (`internal/catalog/signing/tufroot.go`), and that names the directory
   TUF *caches into* — it does not bypass TUF and it will not accept a
   hand-placed `trusted_root.json` as a substitute for a TUF fetch. A name
   like `SHARKO_TRUSTED_ROOT_PATH` is not read by any Sharko build; this
   page claimed one until 2026-08-19.
