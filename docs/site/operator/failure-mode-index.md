...
| EKS token generation failed — `internal/providers/aws_auth.go` `slog.Error("[auth] EKS token generation failed"...)`, twice, told apart by the `step` field (`load-aws-config` / `presign-get-caller-identity`) | **P1** | [`eks-token-generation-failed.md`](eks-token-generation-failed.md) | IRSA misconfigured OR target cluster's role missing `eks:GetToken`. Per-cluster failure. The log line carries no AWS SDK error text (it can carry credential material); the wrapped error is in the API response. Closed by V2-4.3 PR 2b. |
