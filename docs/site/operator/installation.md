...
!!! note "Annotation wording"
    The annotation `sharko.dev/initial-secret: rotated-on-reset-admin` reflects the actual lifecycle. Earlier versions used the wording `delete-after-first-password-change`, which became misleading once `sharko reset-admin` started rotating the secret instead of deleting it. If you see the older annotation on an existing cluster, the next reset-admin (or pod re-bootstrap) will refresh it.
