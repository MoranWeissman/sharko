...
| `DriftDetected` | Warning | Drift detected between the cluster Secret and the Git-defined connection | Reconciler detected that the ArgoCD cluster Secret differs from the connection git defines (credential values resolved from the referenced provider, never stored in Git) |
| **Success Events** | | | |
