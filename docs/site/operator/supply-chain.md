...
Each CLI archive also gets a CycloneDX SBOM (`sharko_X.Y.Z_<os>_<arch>.tar.gz.sbom.json`) generated by [syft](https://github.com/anchore/syft) and published as a release asset next to it — an inventory of every package that went into that binary, useful for checking a specific archive against a known vulnerability. This covers the CLI archives only, not the container image or the Helm charts.
