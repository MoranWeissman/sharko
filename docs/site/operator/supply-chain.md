...
| Per-architecture container image SBOM (`sharko-image-linux-<arch>.cdx.json`) — **configured in the release workflow, not yet produced for any published release** | Cosign CycloneDX attestation on the per-architecture image digest, and the document itself as a release asset | `cosign verify-attestation` |

Each CLI archive also gets a CycloneDX SBOM (`sharko_X.Y.Z_<os>_<arch>.tar.gz.sbom.json`) generated by [syft](https://github.com/anchore/syft) and published as a release asset next to it — an inventory of every package that went into that binary, useful for checking a specific archive against a known vulnerability. An archive SBOM lists Go modules and no Alpine packages, so it says nothing about what is installed in the container image. The image SBOM described in [Verifying the container image SBOM](#verifying-the-container-image-sbom) closes that gap for releases produced from now on; it does not exist for any tag published before it was added.
...
!!! warning "This has not run for any published release yet"
    The release workflow steps that produce, attest and upload these SBOMs were added on 2026-09-24 and have never executed. No tag published before that — `v4.0.1` included — has an image SBOM or an image-SBOM attestation, so the commands below will fail for those tags with "no matching attestations". They are documented now because the workflow change is reviewable today; they become usable the first time a release runs with those steps in place.
...
The same documents are also attached to the release page as `sharko-image-linux-amd64.cdx.json` and `sharko-image-linux-arm64.cdx.json`, which is the convenient way to read one without a registry pull:
