...
- **Connection Data is not reconciled from Git.** Sharko reconciles the cluster Secret's **addon labels** from Git. The connection details themselves — the server URL, CA certificate, and auth token or exec config that live in the Secret's **Data** field — are **not declared in Git**, so Sharko has no Git-desired version to reconcile them against. What Sharko can do instead: on the connection page, **Check again** compares the live connection details against the cluster's configured credentials source — read-only, no write, no self-heal. If they differ, an admin can click **Repair connection** to deliberately rewrite the connection details from that source: a reviewed, manual write pinned to the commit shown at check time, never a background reconcile and never automatic. Connection Data is verified by diagnostics and by this on-demand comparison, not reconciled on a schedule.
