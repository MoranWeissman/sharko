...
- **Connection Data changes are never applied automatically.** Git is the source of truth for the connection definition. Credential values are resolved from the referenced provider during reconciliation and are never stored in Git. Sharko owns and maintains the rendered ArgoCD cluster Secret; ArgoCD consumes it. But automatic sync is deliberately disabled for the sensitive fields: when the connection's Data no longer matches what the Git definition plus the resolved credential values say it should be — a hand edit, a broken token, a rotated backend value — Sharko notices it read-only and does **not** rewrite anything by itself. An admin reviews the difference on the connection page and applies it through the explicit **Repair connection** action, pinned to the reviewed commit. Only the addon labels converge on the reconciler's own schedule.
