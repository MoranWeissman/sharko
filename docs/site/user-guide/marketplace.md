...
2. **Add to your catalog** — an embedded form (NOT a popup) with an explainer:
   > This creates the deployment template ArgoCD uses to roll `<addon>` out to your clusters, and adds an entry to your `addons-catalog.yaml`. The addon will be available to deploy on any cluster afterwards.
