...
The Marketplace is a **read-only browse experience**. Submitting an addon goes through the same GitOps PR flow as every other Sharko mutation — nothing lands in your `catalog.yaml` until a pull request opens (and, if your active connection has `pr_auto_merge` enabled, merges automatically).

> **Two words, everywhere:** the Marketplace is the **browse/discover
> surface** (metadata only, nothing deployed). Adding an addon moves it
> into your **Catalog** — your org's approved list, in `catalog.yaml`.
> Enabling it still requires a separate step per cluster, and now also
> requires the addon to already be in the Catalog. See
...
2. **Add to your catalog** — an embedded form (NOT a popup) with an explainer:
   > This adds a full entry — chart, repo, version, namespace — to your `catalog.yaml`. The addon will be available to enable on any cluster afterwards.
   Fields are **Display name** (defaults to the chart name), **Namespace** (defaults to the chart's recommended namespace), and **Chart version** (top-10 stable picker; tick **Show pre-releases** to expand). The submit button is labelled **Add to catalog**. Sync options, ignore differences, and additional sources are set on the addon page after the PR merges (see [Editing Deployment Options](argocd-app-options.md)).
...
2. **Catalog entry + values.** A branch is created (default prefix `sharko/`), Sharko commits two things atomically:
   - A new, full entry in `catalog.yaml` for `<addon>` — chart, repo, version, namespace, and settings.
   - A generated global values file. The values file is built by the [smart-values pipeline](smart-values.md) — Sharko fetches the chart's upstream `values.yaml` for the version you picked, splits cluster-specific fields into a per-cluster template block at the bottom, and stamps a `# sharko: managed=true` header. If the chart can't be fetched (registry unreachable, version not found), Sharko falls back to a minimal `<name>:\n  enabled: false` stub — you can refresh the file later via the version-mismatch banner once connectivity is back.
   Being in the catalog makes the addon *available* to enable — the engine only creates its ApplicationSet, and only clusters see it, once someone enables it on a cluster (a separate step; see [Managing Addons](addons.md)).
3. **PR opens.** A pull request opens against `catalog.yaml` plus the generated values file.
...
5. **In-memory activity history.** The action is recorded as the existing `addon_added` event with the originating UI flow in the `source` detail field (`marketplace` for curated cards, `artifacthub` for Search results, `manual` for the Add Addon form). Filtering the activity history by source surfaces Marketplace-driven additions vs. manual ones without inventing a new event name.
