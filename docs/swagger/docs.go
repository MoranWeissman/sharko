...
const docTemplate = `{
    ...
    "paths": {
        ...
        "/addons/upgrade-batch": {
            "post": {
                ...
                "responses": {
                    ...
                    "409": {
                        "description": "The connected repo uses the v4 format — use POST /addons/{name}/upgrade-clusters instead",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
        ...
        "/addons/{name}/upgrade-clusters": {
            "post": {
                ...
                "parameters": [
                    ...
                    {
                        "description": "Subset upgrade request",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_orchestrator.UpgradeAddonClustersV4Request"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Upgrade committed (or dry-run preview)",
                        "schema": {
        ...
        "/addons/{name}/values/ai-opt-out": {
            "put": {
                ...
                "responses": {
                    ...
                    "409": {
                        "description": "The repo is still v3-format and needs its migration completed first",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
        ...
        "/addons/{name}/values/annotate": {
            "post": {
                ...
                "responses": {
                    ...
                    "409": {
                        "description": "Addon is opted out of AI annotation, or the repo is still v3-format and needs its migration completed first",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
        ...
        "/audit/stream": {
            "get": {
                ...
                "description": "Opens a Server-Sent Events stream that pushes each new audit entry as it is recorded.\nThe connection stays open until the client disconnects.\nEach SSE \"data:\" line is one JSON audit entry with exactly the shape below — the same shape GET /audit returns in its entries array, \"changes\" field included.",
                "produces": [
                    "text/event-stream"
                ],
                "tags": [
                ...
                "responses": {
                    "200": {
                        "description": "One JSON audit entry per SSE data line",
                        "schema": {
                            "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_audit.Entry"
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
        ...
        "/clusters": {
            ...
            "post": {
                ...
                "description": "Registers a new cluster in ArgoCD and creates its GitOps configuration.\nPass \"dry_run\": true to preview what would happen without making changes.\nProvider may be \"eks\" (default; uses configured secrets provider) or\n\"kubeconfig\" (caller supplies kubeconfig YAML inline via the\n\"kubeconfig\" field — bearer-token auth only).\nOptionally set \"creds_source\" to state explicitly where credentials come\nfrom: \"inline-kubeconfig\", \"secret-kubeconfig\", or \"eks-token\". When omitted\nit is derived: provider \"kubeconfig\" → inline; a pasted \"kubeconfig\" with no\nprovider → inline (the paste is authoritative — it is never silently ignored\nin favour of a backend lookup); anything else → backend. When set it wins\nover \"provider\". The effective source is recorded on the cluster's\nmanaged-clusters.yaml entry as \"credsSource\" and later credential fetches\n(test/diagnose/secrets/addon operations) route by it, so inline-registered\nclusters keep working when a secrets-backend connection is configured.\nOptionally set \"role_arn\" (eks-token creds source only) to record the\nper-cluster IAM role Sharko assumes when minting EKS tokens for this\ncluster — the discovery flow passes the cross-account role that found\nthe cluster so token minting keeps using the same identity. Precedence\nat token-mint time: the structured SM secret's own roleArn, then this\nper-cluster role_arn, then the connection-level provider default.\nOptionally set \"connection_managed_by\" to declare who owns the ArgoCD\ncluster secret: \"sharko\" (default — Sharko writes and rotates it) or\n\"user\" (self-managed — you create the secret by hand; Sharko never writes\nit and only syncs addon labels onto it). Credentials are OPTIONAL at\nregistration for every connection mode (V2-cleanup-88.3 — lazy credentials):\nwhen supplied, connectivity verification still runs; when absent (no\nkubeconfig, no secret_path, no working secrets-provider lookup), it is\nskipped and the cluster registers as connection-only. Sharko only asks\nfor credentials later, when a secret-bearing addon is enabled on the cluster.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
        ...
        "/clusters/adopt": {
            "post": {
                ...
                "responses": {
                    "200": {
                        "description": "Adoption processed. Read outcome/results — 200 does NOT mean every cluster was adopted.",
                        "schema": {
                            "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_orchestrator.AdoptClustersResult"
                        }
                    },
                    "207": {
                        "description": "Some clusters were adopted and some failed outright. Read outcome/results.",
                        "schema": {
                            "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_orchestrator.AdoptClustersResult"
                        }
                    },
        ...
        "/clusters/available": {
            "get": {
                ...
                "responses": {
                    ...
                    "503": {
                        "description": "Credentials provider not configured",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
        ...
        "/clusters/batch": {
            "post": {
                ...
                "responses": {
                    "200": {
                        "description": "Batch processed. Read outcome/results — a 200 only means Sharko accepted and processed the request.",
                        "schema": {
                            "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_orchestrator.BatchResult"
                        }
                    },
                    "207": {
                        "description": "At least one cluster did not fully succeed. Read outcome/results.",
                        "schema": {
                            "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_orchestrator.BatchResult"
                        }
                    },
        ...
        "/clusters/{name}/addons/{addon}": {
            "post": {
                ...
                "description": "Enables a specific addon on a cluster. Updates the cluster values file\n(sets addon to true) and managed-clusters.yaml (sets label to enabled) via PR.\nIf the addon declares secrets, Sharko must have resolvable credentials for\nthe cluster (V2-cleanup-88.3 — lazy credentials): registration succeeds with\nno credentials, but enabling a secret-bearing addon on a cred-less cluster is\nrejected with 422 naming what's missing. Secret-less addons enable with no\ncredential requirement at all. Requires yes=true for confirmation. Pass\ndry_run=true to preview (the credentials gate applies to the preview too).",
                "consumes": [
                    "application/json"
                ],
                "produces": [
        ...
        "/clusters/{name}/addons/{addon}/secret/resource": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                ...
                "responses": {
                    "200": {
                        "description": "The live Secret, values blanked",
                        "schema": {
                            "$ref": "#/definitions/internal_api.secretResourceView"
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                    ...
                    "404": {
                        "description": "No values secret is defined for this addon, or it does not exist on the cluster right now",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    ...
                    "502": {
                        "description": "Sharko could not read the secret",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
        ...
        "/clusters/{name}/connection-comparison": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                ...
                "responses": {
                    "200": {
                        ...
                        "schema": {
                            "$ref": "#/definitions/internal_api.connectionComparisonView"
                        }
                    },
                    "400": {
        ...
        "/clusters/{name}/connection-repair": {
            "post": {
                ...
                "parameters": [
                    ...
                    {
                        "type": "string",
                        "description": "The commit the caller reviewed, from the connection check's compared_commit",
                ...
                "responses": {
                    "200": {
                        "description": "The repair result plus a fresh comparison",
                        "schema": {
                            "$ref": "#/definitions/internal_api.connectionRepairView"
                        }
                    },
                    "400": {
                        "description": "Cluster name or reviewed commit is missing",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    ...
                    "404": {
                        "description": "This cluster is not in the Git-managed cluster list, or it has no ArgoCD connection to repair — nothing was written",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    ...
                    "409": {
                        "description": "The Git branch moved since the caller reviewed it, or the connection changed underneath — nothing was written",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    ...
                    "422": {
                        "description": "This kind of connection cannot be repaired, or is not Sharko's to change",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    ...
                    "502": {
                        "description": "Sharko could not read what the connection should be, so it changed nothing",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    ...
                    "503": {
                        "description": "Sharko is missing something it needs to run the repair",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
        ...
        "/clusters/{name}/orphan": {
            "delete": {
                ...
                "responses": {
                    ...
                    "404": {
                        "description": "Cluster Secret not found in ArgoCD",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
            ...
            "delete": {
                ...
                "responses": {
                    "200": {
                        "description": "Deleted",
                        "schema": {
                            "$ref": "#/definitions/internal_api.orphanedSecretDeleteResult"
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                    ...
                    "404": {
                        "description": "No leftover secret is on record for that cluster, namespace, and name",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    ...
                    "409": {
                        "description": "This secret is claimed by the catalog again, or its ownership marker changed since it was found — nothing was deleted",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    ...
                    "422": {
                        "description": "Could not delete — no Git connection, cluster unreachable, or the delete itself failed",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
        ...
        "/clusters/{name}/reconcile": {
            "post": {
                ...
                "responses": {
                    ...
                    "503": {
                        "description": "The part of Sharko that manages cluster connections is not running on this server",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
        ...
        "/clusters/{name}/resync": {
            "post": {
                ...
                "description": "Re-applies Sharko's own addon-label keys onto this cluster's ArgoCD cluster Secret ONCE, to match Git.\nRegardless of the managed_cluster_self_heal setting, and without reading or changing it.\nTouches only Sharko's own addon-label keys — never foreign labels, Secret Data, or annotations.\nReports the label diff this resync applied (added / removed / changed / unchanged).",
                "produces": [
                    "application/json"
                ],
                "tags": [
                ...
                "responses": {
                    "200": {
                        ...
                        "schema": {
                            "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_models.ClusterResyncResponse"
                        }
                    },
                    "400": {
                        "description": "Cluster has no entry in the Git-managed cluster list",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
        ...
        "/clusters/{name}/secret/resource": {
            "get": {
                ...
                "responses": {
                    ...
                    "404": {
                        "description": "This secret does not exist right now",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    ...
                    "503": {
                        "description": "Sharko has no Kubernetes client for its own cluster on this server",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
        ...
        "/clusters/{name}/secrets/refresh": {
            "post": {
                ...
                "parameters": [
                    ...
                    {
                        "type": "string",
                        "description": "Refresh only this addon's secrets (must be defined in the Git catalog for this cluster)",
                        "name": "addon",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Secrets refreshed from Git",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    ...
                    "207": {
                        "description": "Some secrets refreshed, some failed",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    ...
                    "404": {
                        "description": "The cluster or addon is not defined in Git",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    ...
                    "502": {
                        "description": "Git or upstream error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    ...
                    "503": {
                        "description": "The part of Sharko that delivers addon secrets is not running on this server",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                ...
                "responses": {
                    "200": {
                        ...
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/internal_api.AttentionItem"
                            }
                        }
                    },
                    "500": {
        ...
        "/default-addons": {
            "get": {
                ...
                "responses": {
                    "200": {
                        "description": "Current default addons",
                        "schema": {
                            "$ref": "#/definitions/internal_api.DefaultAddonsResponse"
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
            ...
            "put": {
                ...
                "parameters": [
                    {
                        "description": "New default addon list",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_api.DefaultAddonsPutRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "PR created/updated",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
            ...
            "get": {
                ...
                "responses": {
                    "200": {
                        "description": "Check result",
                        "schema": {
                            "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_orchestrator.EnginePinCheckResult"
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                ...
                "parameters": [
                    {
                        ...
                        "in": "body",
                        "schema": {
                            "$ref": "#/definitions/internal_api.EngineUpgradePinRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Upgrade PR result (or dry-run preview)",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    ...
                    "400": {
                        "description": "Bad request — no engine pin, or already up to date",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
        ...
        "/marketplace/addons/{name}/versions": {
            "get": {
                ...
                "responses": {
                    "200": {
                        "description": "Sorted chart versions for the addon. ` + "`" + `repo` + "`" + ` and ` + "`" + `no_data_reason` + "`" + ` carry the chart repository address with any embedded credential removed.",
                        "schema": {
                            "$ref": "#/definitions/internal_api.catalogVersionsResponse"
                        }
                    },
                    "403": {
                        "description": "Forbidden — requires catalog.freshness.read",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
        ...
        "/notifications": {
            "get": {
                ...
                "description": "Returns recent notifications (upgrades, drift, security, connection health).\nEach notification carries a stable ` + "`" + `code` + "`" + ` — that is the field to branch on.\n` + "`" + `title` + "`" + ` and ` + "`" + `description` + "`" + ` are display text and may be reworded in any release,\nso matching on their wording will break.",
                "produces": [
                    "application/json"
                ],
                "tags": [
        ...
        "/prs/merged": {
            "get": {
                ...
                "responses": {
                    ...
                    "503": {
                        "description": "No usable Git connection. The message is one fixed Sharko sentence pointing at Settings — never the underlying error's own text, which can carry the repository token (B1).",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
        ...
        "/prs/{id}/refresh": {
            "post": {
                ...
                "responses": {
                    ...
                    "502": {
                        "description": "Git provider unreachable",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
        ...
        "/system/managed-secrets": {
            "get": {
                ...
                "parameters": [
                    ...
                    {
                        "type": "string",
                        "description": "Rows filter: exact state match (in_sync, out_of_sync, missing, unknown, foreign, orphaned)",
                        "name": "state",
                        "in": "query"
                    },
                    {
        ...
        "/upgrade/{addonName}/versions": {
            "get": {
                ...
                "responses": {
                    "200": {
                        "description": "Available versions. repo_url carries the chart repository address with any embedded credential removed.",
                        "schema": {
                            "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_models.AvailableVersionsResponse"
                        }
                    },
                    "400": {
                        "description": "Bad request",
                    ...
                    "403": {
                        "description": "Forbidden — requires addon.list",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
            ...
            "post": {
                ...
                "parameters": [
                    ...
                    },
                    {
                        ...
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_orchestrator.EnableAddonV4Request"
                        }
                    }
                ],
                "responses": {
            ...
            "delete": {
                ...
                "parameters": [
                    ...
                    },
                    {
                        ...
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_orchestrator.DisableAddonV4Request"
                        }
                    }
                ],
                "responses": {
        ...
        "/webhooks/git": {
            "post": {
                "description": "Receives GitHub push event webhooks. A request is accepted only when it\ncarries an HMAC-SHA256 signature matching the shared secret an operator\nconfigured. With no shared secret configured the endpoint refuses every\nrequest. An accepted push to the configured base branch triggers a refresh.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
    ...
    "definitions": {
        "github_com_MoranWeissman_sharko_internal_audit.AttributionMode": {
            "type": "string",
            "enum": [
                "service",
                "per_user",
        ...
        "github_com_MoranWeissman_sharko_internal_audit.Entry": {
            ...
            "properties": {
                ...
                "detail": {
                    "description": "semantic detail set by handlers via Enrich",
                    "type": "string"
                },
                "duration_ms": {
                    "type": "integer"
                ...
                "resource": {
                    "description": "cluster:prod-eu, addon:cert-manager",
                    "type": "string"
                },
                "result": {
                ...
                "source": {
                    "description": "ui, cli, api, reconciler, webhook",
                    "type": "string"
                },
                "tier": {
                    "description": "attribution tier of the endpoint",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_audit.Tier"
                        }
                    ]
                },
                "timestamp": {
        ...
        "github_com_MoranWeissman_sharko_internal_audit.Tier": {
            "type": "string",
            "enum": [
                "tier1",
                "tier2",
            ...
            "x-enum-comments": {
                "Tier1": "operational",
                "Tier2": "configuration",
                "TierAuth": "login/logout/hash (no git write)",
                "TierPersonal": "self-service on own profile (no git write)",
                "TierWebhook": "inbound webhook (no user identity)"
            },
            "x-enum-descriptions": [
                "operational",
                "configuration",
                "self-service on own profile (no git write)",
                "login/logout/hash (no git write)",
                "inbound webhook (no user identity)"
            ],
            "x-enum-varnames": [
                "Tier1",
                "Tier2",
        ...
        "github_com_MoranWeissman_sharko_internal_fanout.Outcome": {
            "type": "object",
            "properties": {
                "completed": {
                    "description": "Completed is how many items finished every step.",
                    "type": "integer"
                },
                "failed": {
                    "description": "Failed is how many failed outright — nothing landed for them at all.\nThis is NOT the same number as the older top-level ` + "`" + `failed` + "`" + ` field on\nthe batch response, which counts every item that did not fully\nsucceed, partials included.",
                    "type": "integer"
                },
                "partly_completed": {
                    "description": "PartlyCompleted is how many stopped halfway with nothing rolled back.",
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                ...
                "unrecognized": {
                    "description": "Unrecognized is how many came back with a status this build does not\nknow. Normally zero.",
                    "type": "integer"
                }
            }
        },
        ...
        "github_com_MoranWeissman_sharko_internal_models.AvailableVersion": {
            "type": "object",
            "properties": {
                "app_version": {
                    "type": "string"
        ...
        "github_com_MoranWeissman_sharko_internal_models.AvailableVersionsResponse": {
            "type": "object",
            "properties": {
                "addon_name": {
                    "type": "string"
                ...
                "current_version": {
                    "description": "catalog default version",
                    "type": "string"
                },
                "repo_url": {
                    "type": "string"
                ...
                "versions": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_models.AvailableVersion"
                    }
                }
            }
        },
        "github_com_MoranWeissman_sharko_internal_models.ClusterAddonsAddonSettings": {
            "type": "object",
            "properties": {
                "createNamespace": {
                    "description": "CreateNamespace lets Argo CD create Namespace if it is missing.\nPointer so \"explicitly false\" is distinguishable from \"not set\"\n(the engine's default is true).",
                    "type": "boolean"
                },
                "ignoreDifferences": {
                    "description": "IgnoreDifferences replaces (never merges with) the shipped/delta\nlist, same reasoning as SyncOptions. Modelled as\n[]map[string]interface{} to mirror the existing v3\nAddonCatalogEntry.IgnoreDifferences shape (group/kind/jsonPointers\nobjects) rather than inventing a new typed struct.",
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": true
        ...
        "github_com_MoranWeissman_sharko_internal_models.ClusterResyncLabelDiff": {
            "type": "object",
            "properties": {
                "added": {
                    "type": "array",
        ...
        "github_com_MoranWeissman_sharko_internal_models.ClusterResyncResponse": {
            "type": "object",
            "properties": {
                "cluster": {
                    "type": "string"
                ...
                "label_diff": {
                    "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_models.ClusterResyncLabelDiff"
                },
                "message": {
                    "type": "string"
                },
        ...
        "github_com_MoranWeissman_sharko_internal_notifications.Code": {
            "type": "string",
            "enum": [
                "git_connection_broken",
                "argocd_repo_broken",
                "argocd_auth_failed",
                "argocd_unreachable",
                "argocd_forbidden",
        ...
        "github_com_MoranWeissman_sharko_internal_notifications.Notification": {
            "type": "object",
            "properties": {
                "code": {
                    "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_notifications.Code"
                },
                "description": {
                    "type": "string"
                },
                ...
                "reason": {
                    "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_notifications.Reason"
                },
                "schema": {
                    "type": "integer"
                },
                ...
                "type": {
                    "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_notifications.NotificationType"
                }
            }
        },
        "github_com_MoranWeissman_sharko_internal_notifications.NotificationType": {
            "type": "string",
            "enum": [
                "upgrade",
                "security",
        ...
        "github_com_MoranWeissman_sharko_internal_notifications.Reason": {
            "type": "string",
            "enum": [
                "unreachable",
                "tls",
        ...
        "github_com_MoranWeissman_sharko_internal_orchestrator.BatchResult": {
            ...
            "properties": {
                ...
                "partial": {
                    "description": "Partial is how many of Failed were partials — clusters that changed\nsomething real before stopping. It is a SUBSET of Failed, not a\nseparate bucket, so Succeeded + Failed still equals Total.",
                    "type": "integer"
                },
                "results": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_orchestrator.RegisterClusterResult"
                    }
                },
                "succeeded": {
                    "type": "integer"
        ...
        "github_com_MoranWeissman_sharko_internal_orchestrator.ClusterResult": {
            "type": "object",
            "properties": {
                "addons": {
                    "type": "object",
        ...
        "github_com_MoranWeissman_sharko_internal_orchestrator.DisableAddonV4Request": {
            "type": "object",
            "properties": {
                "addon": {
                    "type": "string"
        ...
        "github_com_MoranWeissman_sharko_internal_orchestrator.EnableAddonV4Request": {
            "type": "object",
            "properties": {
                "addon": {
                    "type": "string"
        ...
        "github_com_MoranWeissman_sharko_internal_orchestrator.EnginePinCheckResult": {
            "type": "object",
            "properties": {
                "bundled_version": {
                    "description": "BundledVersion is this Sharko build's own engine chart version.\nEmpty when V4Repo is false.",
                    "type": "string"
                },
                "message": {
                    "description": "Message is a short, human-readable summary safe to surface directly\nto a user without further formatting.",
                    "type": "string"
                },
                "pinned_version": {
                ...
                "upgrade_available": {
                    "description": "UpgradeAvailable is true when BundledVersion is newer than\nPinnedVersion. Always false when V4Repo is false.",
                    "type": "boolean"
                },
                "v4_repo": {
        ...
        "github_com_MoranWeissman_sharko_internal_orchestrator.RegisterClusterResult": {
            "type": "object",
            "properties": {
                "adopted": {
                    "description": "true if cluster was already in ArgoCD",
                    "type": "boolean"
                },
                "argocd_secret_error": {
                    "description": "ArgoSecretError holds the error message if the ArgoCD secret step failed (non-fatal).",
                    "type": "string"
                },
                "argocd_secret_status": {
                    "description": "ArgoCD cluster secret outcome. Set by the API handler after calling Manager.Ensure().\nPossible values: \"created\", \"adopted\", \"updated\", \"skipped\", \"error\".",
                    "type": "string"
                },
                "cluster": {
                    "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_orchestrator.ClusterResult"
                },
                "completed_steps": {
                    "type": "array",
                    "items": {
                ...
                "dry_run": {
                    "description": "DryRun holds the preview result when dry_run=true. No side effects occur.",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_orchestrator.DryRunResult"
                        }
                ...
                "failed_secrets": {
                    "description": "secrets that failed to create",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_orchestrator.SecretError"
                    }
                },
                "failed_step": {
                    "type": "string"
                ...
                "status": {
                    "description": "\"success\" or \"partial\"",
                    "type": "string"
                },
                "verification": {
                    "description": "Verification holds the Stage1 connectivity verification result (if run).",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_verify.Result"
                        }
        ...
        "github_com_MoranWeissman_sharko_internal_orchestrator.SafeSecretMessage": {
            "type": "object"
        },
        "github_com_MoranWeissman_sharko_internal_orchestrator.SecretError": {
            "type": "object",
            "properties": {
                "addon": {
                    "type": "string"
                ...
                "code": {
                    "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_orchestrator.SecretFailureCode"
                },
                "error": {
                    "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_orchestrator.SafeSecretMessage"
                },
                "key": {
                    "type": "string"
                },
        ...
        "github_com_MoranWeissman_sharko_internal_orchestrator.SecretFailureCode": {
            "type": "string",
            "enum": [
                "SECRET_FETCH_FAILED",
                "SECRET_WRITE_FAILED"
        ...
        "github_com_MoranWeissman_sharko_internal_orchestrator.UpgradeAddonClustersV4Request": {
            "type": "object",
            "properties": {
                "addon": {
                    "type": "string"
        ...
        "internal_api.DefaultAddonsResponse": {
            "type": "object",
            "properties": {
                "addons": {
                    "type": "array",
        ...
        "internal_api.EngineUpgradePinRequest": {
            "type": "object",
            "properties": {
                "auto_merge": {
                ...
                "dry_run": {
                    "description": "DryRun, when true, returns a preview of the diff with no side\neffects. Also settable via ?dry_run=true.",
                    "type": "boolean"
                }
            }
        },
        ...
        "internal_api.NotificationListResponse": {
            ...
            "properties": {
                "notifications": {
                    "description": "Notifications is the requested page, newest first. Route on each\nitem's ` + "`" + `code` + "`" + `; ` + "`" + `title` + "`" + ` and ` + "`" + `description` + "`" + ` are text to show a person and\nmay be reworded in any release.",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_notifications.Notification"
                    }
                },
                "unread_count": {
                    "description": "UnreadCount counts every unread notification, not just this page.",
                    "type": "integer"
                }
            }
        },
        ...
        "internal_api.auditLogResponse": {
            ...
            "properties": {
                "count": {
                    "description": "Count is len(Entries) — how many entries this response carries, NOT\nhow many exist. The ring buffer holds a bounded window and the query\napplies a limit.",
                    "type": "integer"
                },
                "entries": {
                    "description": "Entries are the matching audit entries, newest first.",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_audit.Entry"
                    }
        ...
        "internal_api.connectionCheckStatusView": {
            "type": "object",
            "properties": {
                "interval_seconds": {
                    "description": "IntervalSeconds is the loop's cadence, 0 when no loop is scheduled at\nall on this server.",
                    "type": "integer"
                },
                "last_attempt": {
                    "description": "LastAttempt is RFC3339 of the last pass the loop tried, absent when it\nhas never tried one. ABSENT, never a zero time — the W3-6 rule.",
                    "type": "string"
                },
                "reason": {
                    "description": "Reason is the plain sentence for why checks are not running. Empty\nexactly when Running is true.",
                    "type": "string"
                },
                "running": {
                    "description": "Running is true only when the last attempted pass really checked the\nfleet. It is deliberately not \"a loop object exists\": a loop that skips\nevery pass because git is down is not running checks, whatever its\ngoroutine is doing.",
                    "type": "boolean"
                }
            }
        },
        "internal_api.connectionComparisonDifference": {
            "type": "object",
            "properties": {
                "expected": {
                    "description": "Expected is what Sharko means the field to be. Absent on a sensitive\nfield, always.",
                    "type": "string"
                },
                "live": {
                    "description": "Live is what the connection carries. Absent on a sensitive field,\nalways.",
                    "type": "string"
                },
                "path": {
                    "description": "Path is the field, e.g. \"data.server\" or\n\"metadata.labels[addons.sharko.dev/datadog]\".",
                    "type": "string"
                },
                "sensitive": {
                    "description": "Sensitive marks a field holding sign-in details. When true there is no\nexpected value and no live value in this object at all.",
                    "type": "boolean"
                },
                "status": {
                    "description": "Status is same, different, missing, or unexpected.",
                    "type": "string"
                }
            }
        },
        "internal_api.connectionComparisonNotChecked": {
            "type": "object",
            "properties": {
                "path": {
                    "type": "string"
        ...
        "internal_api.connectionComparisonView": {
            "type": "object",
            "properties": {
                "branch": {
                    "description": "Branch is the configured git branch. Sharko does not discover a default\nbranch.",
                    "type": "string"
                },
                "checked_at": {
                    "description": "CheckedAt is when this check ran, RFC3339.",
                    "type": "string"
                },
                "checked_field_count": {
                    "description": "CheckedFieldCount is how many fields were compared. A count of FIELDS —\nnot of bytes, characters or anything else measured off a value.",
                    "type": "integer"
                },
                "cluster": {
                    "description": "Cluster is the cluster this answer is about.",
                    "type": "string"
                },
                "compared_commit": {
                    "description": "ComparedCommit is the commit every file in this check was read at.\nEmpty when the git provider cannot report a commit — an empty value\nsays \"Sharko does not know\", never a guessed or stale commit.",
                    "type": "string"
                },
                "compared_path": {
                    "description": "ComparedPath is the git file the desired state was read from.",
                    "type": "string"
                },
                "credential_source_type": {
                    "description": "CredentialSourceType is where this cluster's sign-in details are kept:\ninline-kubeconfig, secret-kubeconfig, eks-token, or empty when the\nrecord predates the field. It is a KIND, never a path into a store and\nnever a value.",
                    "type": "string"
                },
                "differences": {
                    "description": "Differences are the fields that did not match, in a fixed order.",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/internal_api.connectionComparisonDifference"
                    }
                },
                "failure_reason": {
                    "description": "FailureReason explains a check_failed answer. Empty otherwise.",
                    "type": "string"
                },
                "limit_reason": {
                    "description": "LimitReason explains a scope narrower than full, in plain words. Empty\nat full scope.",
                    "type": "string"
                },
                "not_checked": {
                    "description": "NotChecked are fields inside the nominal scope Sharko deliberately did\nnot check, each with a reason.",
                    "type": "array",
                    "items": {
                ...
                "ownership_mode": {
                    "description": "OwnershipMode is which of the seven kinds of connection this is.",
                    "type": "string"
                },
                "repair_available": {
                    "description": "RepairAvailable says whether a repair could be offered for this kind of\nconnection. This endpoint never repairs anything.",
                    "type": "boolean"
                },
                "repair_scope": {
                    "description": "RepairScope is exactly what a repair would be allowed to touch: none,\naddon_labels_only, or full_connection.",
                    "type": "string"
                },
                "scope": {
                    "description": "Scope is how much of the connection was checked: full, limited,\naddon_labels_only, or ownership_conflict.",
                    "type": "string"
                },
                "status": {
                    "description": "Status is synced, out_of_sync, missing, check_failed, ownership_conflict\nor limited. synced means every field inside the scope was checked\nsuccessfully AND matched; it is only ever possible at full scope.\ncheck_failed means Sharko could not finish, and is never turned into\nout_of_sync or synced.",
                    "type": "string"
                },
        ...
        "internal_api.connectionRepairView": {
            "type": "object",
            "properties": {
                "branch": {
                    "description": "Branch is the configured git branch. Sharko never discovers a default\nbranch.",
                    "type": "string"
                },
                "cluster": {
                    "description": "Cluster is the cluster this answer is about.",
                ...
                "label_diff": {
                    "description": "LabelDiff reports the addon-label changes for a labels-only repair, the\nsame shape POST /resync reports.",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_MoranWeissman_sharko_internal_models.ClusterResyncLabelDiff"
                        }
                ...
                "message": {
                    "description": "Message is one plain sentence about what happened.",
                    "type": "string"
                },
                "preserved_foreign_data_keys": {
                    "type": "integer"
                },
                "preserved_foreign_labels": {
                    "description": "PreservedForeignLabels and PreservedForeignDataKeys count what the repair\ndeliberately left alone. Counts, not names: naming somebody else's keys\nwould be reporting on their data.",
                    "type": "integer"
                },
                "repaired": {
                ...
                "repaired_at": {
                    "description": "RepairedAt is when the repair ran, RFC3339.",
                    "type": "string"
                },
                "repaired_at_commit": {
                    "description": "RepairedAtCommit is the commit this repair was built from and verified\nagainst. Always set on a successful repair — a repair with an unknown\ncommit is refused rather than performed (see the revision guard).",
                    "type": "string"
                },
                "scope_applied": {
                    "description": "ScopeApplied is what this repair was allowed to touch: full_connection or\naddon_labels_only. It comes from the mode policy, never from the request.",
                    "type": "string"
                },
                "self_heal_unchanged": {
                    "description": "SelfHealUnchanged is always true. Repair does not read the self-heal\nsetting as a precondition, does not write it, and does not depend on it.\nIt is in the body so the promise is visible to anyone reading the API.",
                    "type": "boolean"
                },
                "values_never_returned": {
                    "description": "ValuesNeverReturned is always true, same promise the comparison makes.",
                    "type": "boolean"
                }
            }
        },
        ...
        "internal_api.orphanedSecretDeleteResult": {
            "type": "object",
            "properties": {
                "cluster": {
                    "type": "string"
        ...
        "internal_api.secretResourceKeyView": {
            "type": "object",
            "properties": {
                "key": {
                    "type": "string"
        ...
        "internal_api.secretResourceView": {
            "type": "object",
            "properties": {
                "annotations": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/internal_api.secretResourceLabelView"
                    }
                },
                "api_version": {
                ...
                "created_at": {
                    "description": "CreatedAt is the object's creationTimestamp in RFC3339, \"\" when the\ncluster did not set one. The UI turns this into an age.",
                    "type": "string"
                },
                "data_keys": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/internal_api.secretResourceKeyView"
                    }
                },
                "kind": {
                ...
                "read_from": {
                    "description": "ReadFrom is a plain sentence naming where this object was read from,\nso the panel never leaves a reader guessing which cluster they are\nlooking at.",
                    "type": "string"
                },
                "secret_type": {
                    "description": "SecretType is the Kubernetes Secret type (\"Opaque\",\n\"kubernetes.io/tls\", ...) — metadata, never content.",
                    "type": "string"
                },
                "values_blanked": {
                    "description": "ValuesBlanked is always true. It is in the body so the contract is\nvisible to anyone reading the API, not only to anyone reading this\nfile.",
                    "type": "boolean"
                }
            }
        },
