...
func TestFailBackendRead_ExactText(t *testing.T) {
	...
	if strings.Contains(failBackendRead, "stored sign-in details") {
	...
	for _, banned := range bannedPhrases {
		if strings.Contains(failBackendRead, banned) {
...
func TestFailCredsUnavailable_ExactText(t *testing.T) {
	...
	// Assert the old phrase is absent.
	if strings.Contains(failCredsUnavailable, "stored sign-in details") {
	...
	for _, banned := range bannedPhrases {
		if strings.Contains(failCredsUnavailable, banned) {
...
func TestAuditRepairCredentialFailure_ExactText(t *testing.T) {
	...
	if strings.Contains(sourceText, oldPhrase) {
	...
	for _, banned := range bannedPhrases {
		if strings.Contains(actualAuditLine, banned) {
