...
func (s *Server) repairFullConnection(
	...
	if result.Changed {
		view.Message = fmt.Sprintf("Sharko rewrote %d part(s) of this cluster's connection to match the connection git defines, with credential values resolved from this cluster's configured credentials source. Foreign labels, other data keys and annotations were left alone. The self-heal setting was not changed.", len(result.FieldsWritten))
	} else {
