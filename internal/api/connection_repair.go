...
const (
	...
	// An ALIAS, not a second sentence. It held a byte-identical copy of
	// failNoGitConnection, so the catalog would have shipped one set of words
	// under two identifiers — exactly the ambiguity the identifier scheme
	// exists to remove, and it made the selection sweep unable to say which
	// identifier a response had chosen. Pointing at the one constant keeps the
	// repair-side name that reads naturally at its call sites while there is
	// still only one place the words live.
	...
	// THE TWO MISSING-CONNECTION SENTENCES (B3).
	//
	// There used to be ONE, and it was written on both of this handler's 404
	// paths, and it promised that Sharko builds the connection by itself with
	// no mention of what it would build the connection out of. That promise is
	// true on the full-connection path and FALSE on the labels-only path, so
	// one of the two answers was a lie whichever way it was worded. They are two
	// different facts about two different kinds of connection, so they are two
	// constants — and the name of each one says which.
	//
	// repairFailSecretGoneSharkoCreates is the FULL-CONNECTION path's answer.
	// The promise is true here and the gate is structural: this sentence is only
	// ever reached through RepairScopeFullConnection, which connectioncompare
	// hands out only for a cluster whose credentials source Sharko understands
	// AND whose secrets backend it can read independently of the live Secret.
	// The reconcile pass really does build the connection for such a cluster
	// out of Git and that configured source
	// (internal/clusterreconciler/reconciler.go, createOne). The second sentence is word for word
	// connectioncompare.LimitReasonSecretMissingDurable's, because it is the
	// same promise the connection page makes about the same cluster, and the
	// two must read as one voice. It names no machinery and promises no clock.
	...
	// repairFailSecretGoneLabelsOnly is the LABELS-ONLY path's answer, and it
	// makes no promise about creation at all — because there is no promise that
	// holds across the connections that reach it. A labels-only repair happens
	// on a self-managed connection (Sharko never creates that Secret), a legacy
	// pasted-kubeconfig connection (the credential existed only inside the
	// Secret that is gone), a connection whose recorded credentials source
	// Sharko does not understand, and a backend-stored connection whose backend
	// Sharko cannot read at this moment. What IS true for every one of them is
	// what this sentence says: a labels-only repair re-applies addon labels onto
	// a connection that exists, this cluster has none, and nothing was changed.
	//
	// WHY IT STILL DOES NOT INHERIT connectioncompare.SecretMissingReason,
	// now that both of that function's wrong answers are fixed (B3a, B3b).
	//
	// The original reason is gone: the adopted mapping and the unknown-source
	// refusal were the two answers that would have been false here, and
	// neither exists any more. The decision was re-opened on that basis and
	// comes out the same way, for two reasons that are about THIS response
	// rather than about that function's honesty:
	//
	//  1. It answers a different question. SecretMissingReason says why there
	//     is no Secret. A repair refusal has to say what this repair DID —
	//     "Sharko changed nothing" — and that fact is not in any of those five
	//     sentences, nor should it be: they are also shown on a page where no
	//     repair was asked for.
	//  2. It has to stay one fixed constant. The sentence catalog in
	//     connection_sentences.go collects CONSTANTS, the "Connection
	//     Sentences Up To Date" job generates them into
	//     ui/src/generated/connection-sentences.ts, and the browser tests
	//     import them from there. A refusal body computed per mode is outside
	//     that contract — which is the same reason five inline sentences were
	//     pulled into constants in R2-4.
	//
	// There is no duplicated behaviour to remove: this sentence deliberately
	// carries no per-mode reasoning at all. It states the fact and sends the
	// reader to the check, which is the surface that answers the per-mode
	// question and is already the vocabulary these refusals use.
	...
	// The five sentences below were typed INLINE, in the middle of the two
	// handlers, until R2-4. An inline sentence is invisible to the catalog and
	// to the sweep that checks the catalog is complete, so all five shipped to
	// a person's screen from outside the contract that is supposed to own every
	// sentence on this surface. Only their address changed — not one character
	// of any of them.

	// repairFailAddonLabelsUnknown refuses the repair when Sharko cannot read
	// which addons belong on the cluster. Repairing blind would strip every
	// addon label.
	...
	// The four OUTCOME sentences. Two carry a %d for the number of parts
	// changed, which is a COUNT OF FIELDS — never a length, never anything
...
// @Failure 404 {object} map[string]interface{} "This cluster is not in the Git-managed cluster list, or it has no ArgoCD connection to repair — nothing was written"
...
func (s *Server) handleRepairConnection(w http.ResponseWriter, r *http.Request) {
	...
	audit.Enrich(r.Context(), audit.Fields{
		Event:    lifecycleevents.ClusterConnectionRepairRequested,
	...
	if s.clusterRecon == nil {
		// The audit detail is read by a person, so it says what is missing in
		// the same words the refusal sentence uses — never the component name.
		s.refusedRepairAudit(r, cluster, "refused: the part of Sharko that manages cluster connections is not running on this server")
	...
	if !desired.AddonLabelsKnown {
		...
		writeError(w, http.StatusUnprocessableEntity, repairFailAddonLabelsUnknown)
		return
	}
...
// repairFullConnection rewrites the whole connection to what git defines,
// resolving the credential value from the secrets backend.
func (s *Server) repairFullConnection(
	...
	if result.Changed {
		view.Message = fmt.Sprintf(repairDoneConnectionChanged, len(result.FieldsWritten))
	} else {
		view.Message = repairDoneConnectionUnchanged
	}
...
func (s *Server) repairAddonLabelsOnly(
	...
	// Not found is not an error for labels-only: creating a missing connection
	// is not this handler's job, and doing it here would conflate "repair"
	// with "provision".
	//
	// TWO FACTS, TWO SENTENCES (B3). This site and the 404 the full-connection
	// write raises are NOT the same situation, and for a while they shared one
	// constant that promised Sharko would create the missing connection by
	// itself. That promise holds for the full-connection path and does not hold
	// for a single connection that reaches this one, so the shared sentence was
	// false every time it was sent from here. Each path now sends the sentence
	// that is true for the connections that can reach it — see the two
	// constants at the top of this file.
	if !found {
		s.refusedRepairAudit(r, cluster, "no-op: the ArgoCD connection Secret does not exist yet")
		writeError(w, http.StatusNotFound, repairFailSecretGoneLabelsOnly)
		return
	}
	...
	if outcome.Changed {
		view.Message = fmt.Sprintf(repairDoneLabelsChanged, len(outcome.FieldsWritten))
		...
		view.Message = repairDoneLabelsUnchanged
	}
...
func (s *Server) freshComparisonAfterRepair(
	...
	if !ok {
		return finishView(view, connectioncompare.Compare(connectioncompare.Request{
			ClusterName:  cluster,
			Namespace:    ns,
			CheckFailure: connectioncompare.CheckFailureNoHubClient,
		}))
	}
	...
	if desired.Entry.ExpectedCredentialsRebuildableWithoutLiveSecret(backendCanProvideStoredFacts) {
		...
		if credErr != nil {
			req.CheckFailure = connectioncompare.CheckFailureBackendRead
		} else {
...
func repairRefusal(err error) (status int, message, auditDetail string, tried bool) {
	...
	case errors.Is(err, argosecrets.ErrRepairSecretMissing):
		// ONLY the full-connection write primitive raises this. The
		// labels-only primitive reports a missing Secret as found=false and
		// never as an error, so the creation promise in this sentence cannot
		// reach a guest connection through here. That split is pinned by
		// TestRepairLabelsOnly_MissingSecretIsNotAnError.
		return http.StatusNotFound, repairFailSecretGoneSharkoCreates, "refused: there is no connection to repair", false
...
func (s *Server) refusedRepairAudit(r *http.Request, cluster, detail string) {
	s.repairAudit(r, cluster, lifecycleevents.ClusterConnectionRepairRefused, detail, "rejected", audit.ChangesNone)
}
...
//
// It writes its OWN entry rather than only enriching, because it is the entry
// that names the CATEGORY of failure — Reason — and the middleware's
// request-scoped entry does not carry one. Log.Add turns that category into the
// fixed safe sentence in Error.
//
...
// is a real cost. It is kept because it is the only one that says what kind of
// failure this was.
//
// NOTE (security story S3): the old reason for keeping it — that it was the
// only entry that could tell the audit log to hide the raw error text, without
// which that text would be stored — is gone. No entry can carry error text any
// more: Entry.Error is a SafeText nothing outside internal/audit can build, and
// the sink rebuilds it from the entry's Reason on every write. audit.Fields now
// has a Reason field, so folding this into the middleware entry has become a
// safe refactor rather than a credential hazard. It is left alone here because
// that is a behaviour change to the activity feed, which is not this story's to
// make.
//
...
func (s *Server) auditRepairCredentialFailure(r *http.Request, cluster string, err error) {
	...
	s.auditLog.Add(audit.Entry{
		...
		Changes: audit.ChangesNone,
		// POSITIONAL, NOT TYPE-BASED, AND STILL DELIBERATELY SO.
		//
		...
		// construction, so the category is stated from WHERE we are rather
		// than from what the error turned out to be. The reconciler returns
		// the provider's error unmarked (repair_credentials.go passes
		// GetCredentialsWithOptionalRole's error straight through), so
		// Classify would answer "upstream_failure" for a failure everybody
		// here knows is about credentials.
		//
		// WHAT CHANGED. This used to be a bool that told the sink whether to
		// sanitise, sitting next to a field holding err.Error(). Getting the
		// bool wrong meant the raw text was stored. Now the only thing this
		// line can affect is WHICH catalog sentence a reader gets: there is
		// no error text on the entry to protect, because Entry.Error cannot
		// be written from outside package audit at all. A wrong constant
		// here costs accuracy; it cannot cost safety.
		Reason: audit.ReasonCredentials,
	})
}
