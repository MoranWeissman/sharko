...
// TestRepair_CredentialFailureAuditIsSanitized: the STORED audit entry for a
// credential failure carries the one fixed safe sentence and an empty detail.
//
// What changed with security story S3: the call site no longer sets a flag that
// decides whether to sanitise, and no longer passes any text at all. It names
// the CATEGORY, and audit.Add turns that into the sentence — unconditionally,
// for every entry. So this test now proves the repair path names the right
// category and that the sink does the rest.
func TestRepair_CredentialFailureAuditIsSanitized(t *testing.T) {
	...
	for _, e := range srv.AuditLog().List(1000) {
		...
		if e.Reason != audit.ReasonCredentials {
			t.Errorf("an audit entry about a credential failure has reason %q, want %q", e.Reason, audit.ReasonCredentials)
		}
...
The call site names the category; Log.Add builds the sentence from it, on every entry, with no flag to get wrong.`, e.Error.String())
		}
		if e.Detail != "" {
...
// errorMessageFromBody pulls the one sentence a refusal carries.
func errorMessageFromBody(t *testing.T, body []byte) string {
	t.Helper()
	var decoded struct {
		Error string `json:"error"`
	}
	if err := json.Unmarshal(body, &decoded); err != nil {
		t.Fatalf("decoding the error body %q: %v", string(body), err)
	}
	return decoded.Error
}

// --- B3: the two missing-connection 404s say two different, true things ------
//
// The repair endpoint has two 404 paths for "there is no connection here", and
// they used to send the SAME sentence: one that promised Sharko builds the
// connection by itself, without naming what it would build it out of. That is
// true for a cluster whose credentials Sharko can re-read from its configured
// source, and false for every connection that reaches the labels-only path. The tests below drive the REAL
// handler down each path and assert the exact sentence that comes out.

// TestRepairSecretGone_LabelsOnlyPathsPromiseNothing walks the three
// labels-only connections that can reach a 404 with no live Secret and
// requires the same true, promise-free sentence from each.
//
// A backend-stored cluster whose backend cannot be read also reaches this path
// (connectioncompare drops it to RepairScopeAddonLabelsOnly). It is NOT driven
// here: this fixture's credentials router always answers, and making it refuse
// mid-request would exercise the credential-failure refusal instead. It is
// covered by the classifier's own tests; what matters for this sentence is
// that the constant promises nothing for ANY mode, which the guard test pins.
func TestRepairSecretGone_LabelsOnlyPathsPromiseNothing(t *testing.T) {
	cases := []struct {
		name        string
		managedYAML string
	}{
		{
			// A person maintains this connection. The reconcile pass
			// deliberately never creates the Secret for it — it logs that it
			// is waiting for the user and writes nothing
			// (internal/clusterreconciler/reconciler.go, syncSelfManaged).
			"self managed",
			"clusters:\n- name: " + comparisonCluster + "\n  connectionManagedBy: user\n  credsSource: secret-kubeconfig\n  labels:\n    datadog: enabled\n",
		},
		{
			// The credential was pasted at registration and kept nowhere but
			// the Secret that is now gone, so there is nothing to rebuild it
			// from at all.
			"legacy inline kubeconfig",
			"clusters:\n- name: " + comparisonCluster + "\n  credsSource: inline-kubeconfig\n  labels:\n    datadog: enabled\n",
		},
		{
			// The record predates the credsSource field, which every upgraded
			// install has. Sharko never guesses at a source it does not
			// understand.
			"credentials source not recorded",
			"clusters:\n- name: " + comparisonCluster + "\n  labels:\n    datadog: enabled\n",
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			// No live Secret at all — this is the state under test.
			_, router, argoClient := repairFixture(t, tc.managedYAML, nil, repairFakeVault{})
			...
			// A LITERAL, not the constant. Comparing the response with the
			// constant the handler assigned compares a thing with itself and
			// proves nothing about the words a person reads.
			const want = "There is no ArgoCD connection for this cluster, so there are no addon labels for Sharko to re-apply. Sharko changed nothing. Run the connection check on this cluster to see why the connection is missing and what to do about it."
			got := errorMessageFromBody(t, w.Body.Bytes())
			if got != want {
				t.Errorf("the labels-only 404 sent the wrong sentence:\n got %q\nwant %q", got, want)
			}
			// And the promise itself, in any wording — this is the fact under
			// test, independent of which constant the handler happened to pick.
			for _, promise := range []string{"automatically creates", "sharko creates", "will create"} {
				if strings.Contains(strings.ToLower(got), promise) {
					t.Errorf("the labels-only 404 promised Sharko would build the connection (%q); Sharko will not, or cannot, build it for any connection that reaches this path:\n  %q", promise, got)
				}
			}
			// Nothing may have been written on a refusal.
			if _, err := argoClient.CoreV1().Secrets("argocd").Get(
				context.Background(), comparisonCluster, metav1.GetOptions{}); !apierrors.IsNotFound(err) {
				t.Errorf("a 404 refusal created or wrote a connection Secret (err=%v)", err)
			}
		})
	}
}

// TestRepairSecretGone_FullConnectionPathKeepsThePromise drives a cluster whose
// credentials are stored in a backend Sharko can read — the one kind of
// connection the reconcile pass really does build from Git plus that source —
// with no live Secret, and requires the qualified promise.
func TestRepairSecretGone_FullConnectionPathKeepsThePromise(t *testing.T) {
	_, router, argoClient := repairFixture(t, backendManagedYAML, nil, repairFakeVault{})
	...
	// A LITERAL, for the same reason as above.
	const want = "There is no ArgoCD connection for this cluster to repair. Sharko automatically creates it from Git and the configured credentials source."
	got := errorMessageFromBody(t, w.Body.Bytes())
	if got != want {
		t.Errorf("the full-connection 404 sent the wrong sentence:\n got %q\nwant %q", got, want)
	}
	if _, err := argoClient.CoreV1().Secrets("argocd").Get(
		context.Background(), comparisonCluster, metav1.GetOptions{}); !apierrors.IsNotFound(err) {
		t.Errorf("the repair endpoint created a connection Secret — repair must never provision (err=%v)", err)
	}
}

// TestRepairLabelsOnly_MissingSecretIsNotAnError pins the structural reason the
// full path's creation promise cannot leak onto a guest connection.
//
// Both paths share repairRefusal, and repairRefusal maps
// argosecrets.ErrRepairSecretMissing to the sentence that promises creation. It
// is safe only because the labels-only write primitive reports a missing Secret
// as found=false with a nil error, and never raises that sentinel. If that ever
// changes, a self-managed connection starts being told Sharko will build it.
func TestRepairLabelsOnly_MissingSecretIsNotAnError(t *testing.T) {
	mgr := argosecrets.NewManager(fake.NewSimpleClientset(), "argocd")

	outcome, found, err := mgr.RepairAddonLabelsWithOwnershipCheck(
		context.Background(), comparisonCluster, map[string]string{"datadog": "enabled"}, false)

	if err != nil {
		t.Fatalf("a missing Secret must not be an error on the labels-only path, got %v", err)
	}
	if found {
		t.Error("found should be false when there is no Secret")
	}
	if outcome.Changed {
		t.Error("nothing exists to change, so Changed must be false")
	}
}

// TestRepairAuditDetails_NameNoInternalComponent is the fold-in guard.
//
// An audit record is read by a person, so it obeys the same rule the response
// sentences do: it never names an internal component. One detail said
// "refused: no reconciler on this server". Nothing covered the audit details at
// all, so it could be reworded back and the suite would stay green.
//
// It reads the handler file's string literals rather than a hand-written list
// of details, so a detail added tomorrow is covered without anybody remembering
// to add it here. Comments are deliberately not read: the file explains the
// mechanism to a developer and naming the component there is correct.
func TestRepairAuditDetails_NameNoInternalComponent(t *testing.T) {
	root := repoRootForSweep(t)
	rel := "internal/api/connection_repair.go"
	...
	// "reconciler" as a standalone word. The import path
	// ".../internal/clusterreconciler" is one token and is not a hit.
	word := regexp.MustCompile(`(?i)\breconcilers?\b`)
	...
	ast.Inspect(file, func(n ast.Node) bool {
		...
		if unquoteErr != nil || !strings.Contains(text, " ") {
			return true
		}
		scanned++
		if word.MatchString(text) {
			t.Errorf("%s:%d names an internal component in text a person reads:\n  %q\n"+
				"Say what is missing in the user's terms — e.g. \"the part of Sharko that manages cluster connections\".",
				rel, fset.Position(lit.Pos()).Line, text)
		}
		return true
	})

	if scanned < 20 {
		t.Fatalf("the guard only looked at %d strings in %s — it has lost its reach and would pass vacuously", scanned, rel)
	}
}
