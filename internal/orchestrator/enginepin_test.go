...
// enginePinTestFixture is a minimal-but-real sharko-engine.yaml, same
...
func TestCheckEnginePin_NoFile_NotAV4Repo(t *testing.T) {
	...
	if result.V4Repo {
		t.Error("expected V4Repo=false when sharko-engine.yaml is absent")
	}
...
func TestCheckEnginePin_UpgradeAvailable(t *testing.T) {
	...
	if !result.V4Repo {
		t.Fatal("expected V4Repo=true when sharko-engine.yaml exists")
	}
