// V125-1-9 Story 9.3 — JSON Schema generator.
//
// This file is the reusable core of the cmd/schema-gen binary. Story 9.3
// extracts the generation logic out of cmd/schema-gen/main.go so that:
//
//  1. Tests (generator_test.go in this package, plus the design-doc-example
//     acceptance test) can exercise the generator and validate outputs
//     without spawning a subprocess.
//  2. Story 9.4 can reuse the canonical schema bytes at read time (compile
//     the same generated bytes for runtime validation) if we ever decide to
//     embed schemas via go:embed rather than refetching from disk.
//
// The generator uses github.com/invopop/jsonschema (locked in the plan's
// OQ #1). Key choices and why:
//
