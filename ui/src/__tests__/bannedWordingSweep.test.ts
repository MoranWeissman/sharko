...
const BANNED: { phrase: string; instead: string }[] = [
  {
    ...
    // The ruled replacement sentence is READ from the generated contract
    // (story P5), not typed here. A guidance message quoting a server
    // sentence is still a copy of it: this one would have gone on telling
    // people to write a sentence the server had stopped saying.
    instead:
      'write around "the Git-defined connection". Where a full sentence is needed, the ruled text is exactly: ' +
      `"${CONNECTION_SENTENCES.condComparisonDrift}"`,
  },
]
