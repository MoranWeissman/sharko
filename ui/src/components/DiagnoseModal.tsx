...
/**
 * DiagnoseResultView — the permissions-report body, extracted so the same
 * rendering can be reused both inside DiagnoseModal (fetch-on-open) and
 * inline in the cluster-detail Diagnostics section, where the result must
 * PERSIST instead of fading with a modal (HD1, V3). Presentational only:
 * it takes a report and owns just the local copy-button state.
 */
export function DiagnoseResultView({ report }: { report: DiagnosticReport }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        ...
        {report && !loading && <DiagnoseResultView report={report} />}
      </DialogContent>
    </Dialog>
  );
}
