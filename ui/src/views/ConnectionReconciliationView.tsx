...
//     no UI-computed "Sharko will fix this …" promise anywhere, in any
//     wording — the ban is on the browser writing one at all, not on the one
//     phrasing it happened to use first.
...
// ─────────────────────────────────────────────────────────────────────────────
// There are no headline or qualifier constants in this file (story P5).
//
// Twelve HEADLINE_* and three QUALIFIER_* constants used to sit here, each
// one a hand-typed copy of a sentence the server authors. No shipped code
// path read a single one of them: the server sends sync.headline and
// sync.qualifier and this page renders those verbatim, so the constants
// existed only for the test file to import as its expected values. That is a
// copy of server wording living in shipped browser code — it drifts silently
// the moment the Go sentence is reworded, and nothing goes red.
//
// They are DELETED rather than replaced with generated imports. Shipped code
// that renders what it received needs no copy of the words at all, and a
// generated import here would still put 15 server sentences in the shipped
// bundle for no rendering purpose. The test now reads the same values out of
// ui/src/generated/connection-sentences.ts, which cmd/gen-connection-sentences
// writes from the Go catalog and CI's "Connection Sentences Up To Date" job
// keeps honest.
//
// The sync headline names its scope (product correction 4): bare "Synced"
// never renders for any connection whose full owned scope was not verified.
// That rule is enforced server-side, in the canonical core.
// ─────────────────────────────────────────────────────────────────────────────

/**
 * The health words, and the lookup that reads them.
 *
 * ONE TABLE FOR BOTH SURFACES, and it is not this file's. This page and the
 * fleet list each used to hold their own copy of the same four words and
 * their own copy of the same lookup, held together by a comment in each file
 * saying they agreed. Nothing enforced it, and the tests pinned the two
 * tables separately — so an edit to either one alone would have left the
 * same connection reading one word in the list and another on this page,
 * with everything green. The table moved to ./connectionHealthWords, which
 * is now the only place either word is written down.
 *
 * The names stay: this page has said HEALTH_WORDS / healthWordFor since B13
 * and its tests import them by those names, so the re-export keeps the
 * page's own vocabulary while removing the second copy.
 */
export const HEALTH_WORDS = CONNECTION_HEALTH_WORDS
export const healthWordFor = connectionHealthWord
...
/**
 * (B13 item 6) The one name for "run the check for this connection now",
 * used by the page's own check button and by the button offered when the
 * endpoint refused. The refusal button used to read "Try again", which is a
 * different name for the identical action.
 */
...
/**
 * Which management modes the server can ONLY reach by really classifying this
 * connection, and which ones it also hands out as a default when it could not.
 *
 * This is the whole basis of the F7 exception below, so it is written down as
 * typed state and not guessed from the names. Read off the server at the
 * commit this page is built against (internal/api/connection_reconciliation.go
 * managementModeFor, internal/connectioncompare/mode.go Classify):
 *
 *  - foreign_owned — only ever returned from Classify's first rule, which
 *    fires on the live Secret's ownership label naming another tool. No
 *    fallback and no not-checked path can produce it: knowing somebody else
 *    owns the Secret requires having read it.
 *  - self_managed — only ever returned from Classify's self-managed or adopted
 *    rules, off the git record's connectionManagedBy or the live Secret's
 *    adopted marker. Never a fallback value either.
 *  - legacy_inline and sharko_managed — returned by real classification AND by
 *    managementModeFor's fallback, which runs when a check failed before any
 *    classification happened and picks legacy_inline from the recorded
 *    credential source, sharko_managed otherwise. On a failed check either of
 *    them may be nothing more than that default.
 *
 * A Record over the union rather than a list: adding a fifth management mode
 * to the wire type is then a compile error here, not a silent false.
 */
const MODE_IS_CLASSIFIED: Record<ConnectionReconciliation['management_mode'], boolean> = {
  foreign_owned: true,
  self_managed: true,
  legacy_inline: false,
  sharko_managed: false,
}

/**
 * The one line the folded routine successes render as.
 *
 * ONE FUNCTION FOR TWO PLACES. The collapsed button and the expanded button
 * used to carry the same expression twice, and it was a counting template with
 * a hard-coded plural noun — so the ordinary foreign-owned state, which has
 * exactly one routine condition since the ownership row was dropped, rendered
 * "All 1 checks passed." on a page a person actually walks through.
 *
 * The plural sentences are byte-for-byte what they were; only the
 * one-condition case is new.
 *
 * ZERO, TOO. `routineChecksSummary(0, true)` returned "All 0 checks passed."
 * — a claim that checks ran and passed, on a page where nothing ran. It is
 * the exact sibling of the "All 1 checks passed." defect above, left behind
 * because both render sites happen to sit behind `routineConditions.length >
 * 0` today. That guard is one edit away from not being there, which is the
 * same distance the plural noun was. A function that can only be called
 * correctly is not fixed; this one is total.
 */
export function routineChecksSummary(count: number, allConditionsOK: boolean): string {
  if (count <= 0) return 'No routine checks.'
  if (count === 1) return '1 check passed.'
  return allConditionsOK ? `All ${count} checks passed.` : `${count} checks passed.`
}
...
// them verbatim.
//
// The HEADLINE_* / QUALIFIER_* constants that survived that deletion as the
// tests' expected values are gone too (story P5) — see the note at the top
// of this file. The expectations now come from the generated contract, so
// nothing in this file holds a copy of a server sentence.
// ─────────────────────────────────────────────────────────────────────────────
...
export function ConnectionReconciliationView({
  ...
  const checkLabel = view?.sync.checked_at ? 'Check again' : CHECK_NOW_LABEL
  const checkButton = (
  ...
  if (error && !view) {
    return (
      <div className="space-y-2" data-testid="recon-error">
        ...
        {/* B13 item 6: "Check now" — the same name the row menu and the
            page's own check button use. This button does exactly what they
            do; a second name for one action is a second thing to learn. */}
        <ActionButton onClick={recheck} loading={loading} loadingLabel="Checking…" icon={RefreshCw} label={CHECK_NOW_LABEL} testId="recon-retry" />
      </div>
    )
  }
  ...
  //
  // THE ONE EXCEPTION, and it is a guard, not a derivation. A connection may
  // not be called synced unless every field inside the scope Sharko owns was
  // actually compared. The fleet row has refused a synced word at a
  // non-full verification scope since B13 (rowStatus in ManagedSecrets.tsx),
  // and this page did not — so one response could put "Connection synced" on
  // this page while the same connection's row in the list read "Not checked
  // yet". Two surfaces disagreeing about one connection is the defect this
  // whole round has been removing, and having the guard on only one of them
  // is what produced the disagreement.
  //
  // The server enforces the invariant in its canonical core and now warns
  // when it fires, so this should never run. It is here for the same reason
  // the fleet's copy is: an enforcement point is one edit away from being
  // forgotten, and the cost of being wrong is telling somebody their
  // connection is fine when nothing looked at it.
  //
  // It can only ever DOWNGRADE. It never invents a better state than the
  // server sent, and the word it falls back to is the SERVER's own
  // not-checked headline from the generated contract — not a sentence
  // written here.
  const syncedWordUnsupported = view.sync.state === 'synced' && view.sync.verification_scope !== 'full'
  const headline = syncedWordUnsupported ? CONNECTION_SENTENCES.headlineNotCheckedYet : view.sync.headline
  ...
  // F7 suppresses the mode statement on a failed check, because a mode the
  // server DEFAULTED to before it classified anything is cosmetic. That is
  // right for the modes it can default to, and wrong for the two it cannot —
  // see MODE_IS_CLASSIFIED. On a foreign-owned connection whose credentials
  // backend could not be read, the blanket suppression left the reader with a
  // "Take ownership" button and no sentence anywhere saying why.
  //
  // Decided on typed state — the management mode — never by comparing this
  // sentence against another one.
  const modeStatementSuppressed = checkFailed && !MODE_IS_CLASSIFIED[view.management_mode]
  ...
  const healthWord = healthWordFor(view.health.state)
  ...
  return (
    <div className="space-y-4" data-testid="recon-view">
      ...
      {/* The page's model sentence — once, small. Not rendered when the check
          itself failed AND the mode is one the server can default to (review
          finding F7): a mode derived before classification is cosmetic, not
          authoritative. A mode it can only reach by classifying is stated
          whatever happened to the check. */}
      {!modeStatementSuppressed && (
          ...
          (conditionsExpanded ? (
            <>
              ...
              >
                ...
                {routineChecksSummary(routineConditions.length, allConditionsOK)}
