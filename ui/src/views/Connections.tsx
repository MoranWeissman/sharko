...
function formFromConnection(conn: ConnectionResponse): ConnectionFormData {
  ...
  return {
    ...
    provider_type: (conn.provider?.type || '') as '' | 'aws-sm' | 'k8s-secrets',
    provider_region: conn.provider?.region || '',
    provider_prefix: conn.provider?.prefix || '',
    gitops_base_branch: conn.gitops?.base_branch || 'main',
    gitops_branch_prefix: conn.gitops?.branch_prefix || 'sharko/',
    gitops_commit_prefix: conn.gitops?.commit_prefix || 'sharko:',
    gitops_pr_auto_merge: conn.gitops?.pr_auto_merge ?? false,
    gitops_host_cluster_name: conn.gitops?.host_cluster_name || '',
    gitops_default_addons: conn.gitops?.default_addons || '',
  }
}
