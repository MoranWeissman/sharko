...
import type { LifecycleEvent } from '@/generated/lifecycle-events'
 ...
 *
 * TYPED ON THE GENERATED CONTRACT, and that is the whole safety net. The keys
 * were plain strings hand-copied from Go, and the test that was supposed to
 * catch a server rename compared them against a SECOND hand-typed copy of the
 * same list, in the test file. The two agreed with each other and with
 * nothing else — no .go file and no CI job referred to either — so a rename
 * on the server would have quietly stopped an event from ever rendering.
 * That matters here more than in most tables, because there is no fallback:
 * an unrecognised event is skipped, so a rename does not show up as a wrong
 * word, it shows up as nothing at all, and enough of them leave this page
 * saying "Nothing recorded since Sharko started." when things did happen.
 *
 * Record<LifecycleEvent, …> means the type-check fails if the server declares
 * an event with no title here, or if a name this table uses is no longer
 * declared. LifecycleEvent comes from ui/src/generated/lifecycle-events.ts,
 * which cmd/gen-lifecycle-events writes from internal/lifecycleevents and
 * CI's "Lifecycle Events Up To Date" job keeps honest.
 *
 * The KINDS are kept in step with lifecycleEventCatalog in
 ...
 * same thing.
 */
export const ACTIVITY_EVENT_TITLES: Record<LifecycleEvent, ActivityEventMapping> = {
  ...
  // This title is spelled the same as a server headline for a connection's
  // state. It is not a copy of it — this table titles a write that already
  // happened. Classified in ui/src/__tests__/browserOwnedCopy.ts.
...
export function mapActivityEntries(entries: AuditEntry[]): ActivityFeedItem[] {
  ...
  for (const entry of entries) {
    const mapping = ACTIVITY_EVENT_TITLES[entry.event as LifecycleEvent] as ActivityEventMapping | undefined
